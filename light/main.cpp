...
int main(int argc, char **argv) {
  ...
  std::filesystem::path max_brightness_file =
      options->backlight_directory / "max_brightness";
  double max_brightness = read_float(max_brightness_file);

  int brightness = options->brightness_percent * max_brightness;

  std::filesystem::path brightness_file =
      options->backlight_directory / "brightness";
